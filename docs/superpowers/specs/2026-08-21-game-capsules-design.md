...
```
STEAM_CAPSULE_URL = "https://shared.cloudflare.steamstatic.com/store_item_assets/steam/apps/{appid}/header.jpg"
...
One constant, so a CDN move is a one-line change (the legacy
`cdn.cloudflare.steamstatic.com/steam/apps/<appid>/header.jpg` still
redirects today). The 460×215 "header" asset is the capsule used on the store
and is the only size we need — CSS scales it.

## 2. Rendering

- **Game page** (`games/game_detail.html`): the existing cover `<img>` switches
  to `capsule_url`, `loading="lazy"`, `alt=""` (decorative — the title is the
  text), and `onerror="this.remove()"` so a dead CDN URL leaves no broken-image
  icon. Width capped by one functional rule (`.capsule { max-width: 460px;
  height: auto; }`).
