...
- Contribution `status` enum (`active`/`disputed`/`removed`) exists from day one, dormant. `disputed` is never shown publicly; it freezes confirmations during review (future moderation flow).
...
- Explicit consent at signup with clear purposes; EU hosting (simplifies the file — no extra-EU transfer paperwork).
