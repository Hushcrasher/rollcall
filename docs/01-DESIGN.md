...
- **Game capsule** (added 2026-08-21, spec `docs/superpowers/specs/2026-08-21-game-capsules-design.md`): the game page renders `Game.capsule_url` — the catalog's own `cover_url` when the seed's Steam header or the IGDB import supplied one, else derived from Steam's public CDN header by `steam_appid` (one constant, `STEAM_CAPSULE_URL`, so a CDN move is a one-line change), else nothing shown. No bytes ever pass through Rollcall either way. The `<img>` carries `referrerpolicy="no-referrer"` (a Steam-derived request shouldn't announce Rollcall URLs to a third party) and `onerror="this.remove()"` — the one inline handler in the templates — so a dead CDN URL leaves no broken-image icon. Posture: hot-linking Steam's own public store asset is exactly what the store pages themselves do; the owner accepted this Steam-derived use knowingly on 2026-08-21.
...
A contribution = **(person, game, optional employer company, discipline, free-text job title, start date, end date, optional country)**.
...
- The company on a contribution is the person's **employer** (e.g. "worked on Dark Souls, at Virtuos" — outsourcing is a huge, invisible part of the industry and exactly the audience this platform serves). The game's own developer/publisher comes from IGDB separately. **Entry is a picker, not search-first** (changed 2026-08-21, spec `docs/superpowers/specs/2026-08-21-credit-form-v2-design.md`): once a game is picked, the form shows a `<select>` of that game's own credited companies in role order (developer → publisher → porting → support, deduplicated), **developer preselected** — the common case ("I worked at the studio that made it") takes no click. Trailing options are `No employer / freelance` and `Another company…`, the latter revealing the existing company search (autocomplete, plus the "create company" offer for outsourcing studios missing from IGDB). Editing a credit preselects the already-saved company even when it isn't one of the game's own credited studios — and a credit saved **without** an employer keeps none: the developer-first default applies only when the form has never asked (a new credit), never when the answer was recorded as empty, so re-saving a freelance credit cannot stamp a studio on it.
...
- **Dates: month + year precision** (stored as regular DATE columns with day forced to 01 — native SQL overlap operations matter for the future vouching system). `end_date` nullable = "still working on it". **Entered as `MM/YYYY` text** (changed 2026-08-21, spec `docs/superpowers/specs/2026-08-21-credit-form-v2-design.md`): a numeric-keypad text input (placeholder and pattern `MM/YYYY`), not the native `<input type="month">`, which rendered in the visitor's own browser locale ("février 2026") — inconsistent with the site's own MM/YYYY display (§3.6). The legacy `YYYY-MM` format is still accepted on input (older clients, existing tests); an invalid value shows `Enter a month as MM/YYYY, e.g. 08/2024.`
- **Country: optional, per credit** (added 2026-08-21, same spec): where the work happened — independent from the person's own profile country, since a career can span several. Asked after the dates on `/credits/new/`, credit edit and the declare funnel's details step; shown after the dates on the profile and game credit lines (`08/2024 – present · France`) and included in the JSON export (§3.7). Not shown on the OG card (which stays the owner's own location, §3.4) and not yet a recruiter filter — tracked in ROADMAP.md.
...
- **Capsule thumbnails on credit lines** (same 2026-08-21 game-capsules spec): each credit line on the profile page — and each game on a company page's game list — shows a small 92×43 capsule (`Game.capsule_url`, see §3.1) to its left, same `no-referrer`/`onerror` guards as the game page, only when a URL exists; the line keeps its plain shape otherwise.
...
- **Credit dates display as `MM/YYYY` sitewide** (changed 2026-08-20, same spec): profile, game, search-result and feed date ranges all read like `08/2024–03/2025` (`present` for an open end). Career-summary ranges on result cards (`2021–present`) and game release years stay years — they are years, not month/year contributions. **Entry is `MM/YYYY` text** (changed 2026-08-21, spec `docs/superpowers/specs/2026-08-21-credit-form-v2-design.md`), not the native `<input type="month">` — see §3.3.
