"""ProfileForm.github_url — parse to a login and manage the cache."""
...
def test_valid_url_is_stored_as_login() -> None:
    user = _user()
    form = ProfileForm(_data(github_url="https://github.com/torvalds"), instance=user)
...
def test_repo_url_keeps_first_segment() -> None:
    user = _user()
    form = ProfileForm(_data(github_url="github.com/torvalds/linux"), instance=user)
    assert form.is_valid(), form.errors
    form.save()
    user.refresh_from_db()
    assert user.github_login == "torvalds"
...
def test_invalid_url_is_a_validation_error() -> None:
    form = ProfileForm(_data(github_url="not a url !!"), instance=_user())
...
def test_changing_the_handle_wipes_the_cache() -> None:
    ...
    form = ProfileForm(_data(github_url="github.com/gvanrossum"), instance=user)
    assert form.is_valid(), form.errors
    form.save()
