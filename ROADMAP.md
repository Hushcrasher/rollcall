...
- [ ] **No structural guard against a test making a network call.** One test on the `feat/igdb-auto-fallback` branch stubbed `IGDBClient.get_game` but not `search_games`, and the untouched code path reached `get_context_data` → `_offer_igdb_matches` → `cached_search` → a live `POST https://id.twitch.tv/oauth2/token` — caught only by review, not by the suite. `conftest.py` carries no fixture that fails a test making an outbound connection; adding an autouse one (e.g. patching `socket.socket` to raise, scoped to `tests/`) would close this permanently instead of relying on every future IGDB-adjacent test remembering to stub the right method.
