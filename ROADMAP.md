...
- [x] **The filter bento becomes a filled surface** (2026-08-24): drawn as one solid object with white text instead of an outline. **`#4E609E` is not a third colour** — it is the hover shade already in the palette, promoted to a surface, and it was picked on measurement rather than taste: white on the highlight `#647BCA` is **4.02:1**, under the 4.5:1 that the form's 12px bold labels need, while this is **6.02:1**. `.muted` drops to `opacity: .8` inside the bento (4.53:1, still a visible step below the labels) because its .7 was calibrated against white and fell to 3.88:1 here. Every text element in the card was measured after the fact: labels, legends, chips, `OR` and the checkbox label at 6.02:1, the group headings and the data caveat at 4.53:1 — all over the line. Two implementation notes worth keeping: the bento **rebinds `--pico-color`** rather than setting `color`, because Pico writes `color: var(--pico-color)` onto `<p>`, `<ul>`, `<label>` and more, so setting `color` alone left the group headings and chip labels oxblood on blue (found by walking the ancestor chain to see where white turned back, not by guessing which tags Pico paints); and the submit button needs **literal** values, because Pico redefines `--pico-background-color` on the button itself, so reading that variable there returned the highlight and left the button blue. The dropdown panel rebinds the variable back — it keeps a white ground of its own and would otherwise have been white on white.
